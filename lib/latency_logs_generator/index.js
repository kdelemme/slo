...
let documentCount = 0;
let documentsPerDay = (60 / INTERVAL) * 60 * 24;

function generateData(date, isPastData = false) {
  documentCount++;
  ...
  switch (SCENARIO) {
    ...
    case "99percent_good":
      ...
      break;
    case "full_outage_every_day":
      // https://www.desmos.com/calculator/kpilzqplg7
      isGood = 10 * Math.cos(documentCount / 5000) + 9.9 > 0 ? true : false;
      break;
    case "healthy_then_failing":
      isGood = isPastData ? true : false;
      break;
    default:
...
async function generateNewData(toDate) {
  ...
  while (true) {
    ...
    if (isAfter(now, lastDate)) {
      lastDate = now;
      const data = generateData(now, false);
      ...
      console.log(
        `[%s] inserted 1 document with latency [%d], host [%s]`,
        now.toISOString(),
        data.latency,
        data.host
      );
